...
class MedDataset(Dataset):
    def __init__(self,
        ...
        if not only_normal:
            ...
            self.fewshot_abnorm_img, self.fewshot_abnorm_mask = [], []
...
class MedValDataset(Dataset):
    """(Added) validation dataset for few-shot learning. 
    It is basically the same as MedDataset, but takes data from /valid folder,
    does not include the few-shot images as atributes.
    NB: load_ds_folder function is modified so that it discards the few-shot samples.
    """
    def __init__(self,
                 dataset_path='/data/',
                 class_name='Brain',
        ...
        self.fewshot_norm_names = self.get_few_normal_names()
        if not only_normal:
            self.fewshot_abnorm_names = self.get_few_abnormal_names()
        else:
            self.fewshot_abnorm_names = []
    ...
    def load_dataset_folder(self):
        ...
        img_fpath_list = sorted([os.path.join(normal_img_dir, f) for f in os.listdir(normal_img_dir)
                                 if f not in self.fewshot_norm_names])
        x.extend(img_fpath_list)
        y.extend([0] * len(img_fpath_list))
        mask.extend([None] * len(img_fpath_list))

        # abnormal
        abnormal_img_dir = os.path.join(self.dataset_path, 'valid', 'Ungood', 'img')
        img_fpath_list = sorted([os.path.join(abnormal_img_dir, f) for f in os.listdir(abnormal_img_dir)
                                 if f not in self.fewshot_abnorm_names])
        x.extend(img_fpath_list)
        y.extend([1] * len(img_fpath_list))
    ...
    def get_few_abnormal_names(self):
        fnames = []
